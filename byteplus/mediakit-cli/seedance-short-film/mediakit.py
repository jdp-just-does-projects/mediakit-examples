"""Subprocess wrapper around `mediakit-cli`, pointed at BytePlus AI MediaKit.

    npx @volcengine/mediakit-cli install -y     # Node >= 18 (same CLI for both clouds)
    export MEDIAKIT_API_KEY=...                  # BytePlus AI MediaKit console -> Settings -> API key
    # MEDIAKIT_ENDPOINT defaults to https://mediakit.ap-southeast-1.bytepluses.com (config.MEDIAKIT)

One cloud tool is used, `video enhance-video` (upscale), polled through
`shared query-task`; stitching is `--local editing concat-video` (ffmpeg on
this machine) because BytePlus does not enable `concat-video`, and inputs are
public URLs because BytePlus does not enable the CLI's upload tool either.
...
def _env() -> dict[str, str]:
    ...
    # Per-process override; the CLI's global config (and its Volcano Engine default) is untouched.
    env.setdefault("MEDIAKIT_ENDPOINT", MEDIAKIT["endpoint"])
    return env
...
def enhance_video(
    ...
    *,
    ...
    """Submit a cloud upscale. `video` must be a publicly reachable http(s) URL: the BytePlus
    deployment does not enable the upload tool the CLI uses for local paths."""
    if not video.startswith(("http://", "https://")):
        raise ValueError(f"enhance_video needs a public URL on BytePlus (no upload tool); got {video!r}")
...
def concat_video_local(videos: list[str], output_path: str, *, timeout: float = 600) -> dict:
    """Synchronous local concat (ffmpeg under the hood; no API key, no endpoint). Returns the
    flat result JSON: {"video_url": <local path>, "duration": ..., "resolution": ...}."""
