"""Idea -> screenplay -> character sheets -> Seedance 2.5 clips -> MediaKit upscale -> local stitch.

    export ARK_API_KEY=...                # BytePlus ModelArk key (LLM / Seedream / Seedance)
    export MEDIAKIT_API_KEY=...           # BytePlus AI MediaKit key (upscale)
...
CLIP_URL_MAX_AGE_S = 22 * 3600  # same 24h clock for Seedance clip URLs, which BytePlus MediaKit must fetch itself
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument("--skip-enhance", action="store_true", help="stitch the raw 480p clips (no MEDIAKIT_API_KEY needed)")
    ...
    p.add_argument("--mediakit-schema", action="store_true", help="print enhance-video --schema and exit")
...
def load_or_init_state(out: Path, args) -> dict:
    ...
    state = {
        ...
        "mediakit_endpoint": os.environ.get("MEDIAKIT_ENDPOINT") or mediakit.MEDIAKIT["endpoint"],
...
def print_dry_run(sp: sp_mod.Screenplay, state: dict) -> None:
    ...
    print(json.dumps({
        ...
        "mediakit": {
            "endpoint": state.get("mediakit_endpoint"),
            "enhance": f"mediakit-cli --cloud video enhance-video --video-url <seedance video_url of shot N> --scene {cfg['enhance_scene']} "
                       f"--tool-version {cfg['enhance_tool_version']} --resolution {cfg['enhance_resolution']} --bitrate-level {cfg['bitrate_level']}",
            "concat": "mediakit-cli --local editing concat-video --video-urls enhanced/shot_1.mp4,... --output-path final.mp4",
        },
...
def step_shots(out: Path, state: dict, sp: sp_mod.Screenplay, args) -> None:
    ...
    def on_update(task_id: str, task: dict) -> None:
        ...
        if url := ark.video_url_of(task):
            e["video_url"] = url
            e.setdefault("video_url_at", time.time())
        if task.get("error"):
            e["error"] = task["error"]
        save_state(out, state)
...
def clip_source_url(entry: dict) -> str:
    """The URL BytePlus MediaKit will download the clip from. BytePlus does not enable the
    CLI's upload tool, so the only source we have is the Seedance result URL (24 h)."""
    sd = entry["seedance"]
    url = sd.get("source_url_override") or sd.get("video_url")
    if not url:
        raise SystemExit(f"shot {entry['index']} has no Seedance video_url in state.json; nothing to enhance from")
    if not sd.get("source_url_override"):
        age = time.time() - sd.get("video_url_at", 0)
        if age > CLIP_URL_MAX_AGE_S:
            raise SystemExit(
                f"shot {entry['index']}: its Seedance URL is {age/3600:.1f}h old and Ark URLs expire at 24h. BytePlus "
                f"MediaKit cannot take a local upload, so re-host {sd.get('local_path')} on a public URL and put it in "
                f"state.json as shots[{entry['index']-1}].seedance.source_url_override, then re-run.")
    return url
...
def step_enhance(out: Path, state: dict, args) -> None:
    ...
    for entry in state["shots"]:
        ...
        src = clip_source_url(entry)  # BytePlus MediaKit fetches the Seedance URL itself (no upload tool)
...
def step_concat(out: Path, state: dict, args) -> None:
    """Stitch with `mediakit-cli --local editing concat-video` (ffmpeg). BytePlus AI MediaKit does
    not enable the cloud `concat-video` tool, so this step is always local and synchronous."""
    cc = state["concat"]
    final = out / "final.mp4"
    ...
    state["concat"] = {"mode": "local", "input_list": inputs, "completed_at": now_iso(),
                       "duration": result.get("duration"), "resolution": result.get("resolution"), "local_path": "final.mp4"}
    write_log(out, "mediakit_concat_final.json", result)
    mark_done(out, state, "concat")
    log(f"[concat] local concat -> final.mp4 ({result.get('duration')}s, {result.get('resolution')})")
...
def main(argv=None) -> int:
    ...
    if args.mediakit_schema:
        print(json.dumps({"enhance-video": mediakit.schema("video", "enhance-video")}, indent=2, ensure_ascii=False))
        return 0
    ...
    if reaches_mediakit:  # fail before any paid Ark call
        log(f"[pipeline] mediakit-cli {mediakit.preflight(require_key=not args.skip_enhance)}")
